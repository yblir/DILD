...
# from datasets.video_dataset_stil import *
...
def main():
    ...
    # todo 加大真实样本权重
    criterion = torch.nn.CrossEntropyLoss(weight=torch.FloatTensor([4.0, 1.0]).cuda()).cuda(args.local_rank)
    ...
    if args.model.resume:
        ...
        print(getattr(args.model, 'only_resume_model', False))
...
def test(dataloader, model, criterion, optimizer, epoch, global_step, args, logger):
    ...
    gather_y_outputs = torch.stack(y_outputs)
    gather_y_labels = torch.stack(y_labels)
    # compute accuracy metrics
    ...
    logger.info(
            '[TEST] EPOCH-{} Step-{} ACC: {:.4f} RealACC: {:.4f} FakeACC: {:.4f}  lr: {:.7f}'.format(
                    epoch, global_step, acc, real_acc, fake_acc, lr
            )
    )
    if args.local_rank == 0:
        ...
        checkpoint_save_name = \
            "Epoch-{}-Step-{}-ACC-{:.4f}-RealACC-{:.4f}-FakeACC-{:.4f}-LR-{:.6g}.tar".format(
                    epoch, global_step, acc, real_acc, fake_acc, lr
            )
...
if __name__ == '__main__':
    ...
    #                     # default="/mnt/e/DeepFakeDetection/FakeDatasets/FaceForensics++",)
