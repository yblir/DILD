...
class FFPP_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in sampled_idxs:
            ...
            # sampled_idxs_new.extend([real_idxs[i - 2], real_idxs[i - 1], real_idxs[i], real_idxs[i + 1], real_idxs[i + 2]])
            sampled_idxs_new.extend([real_idxs[i - 1], real_idxs[i], real_idxs[i + 1], real_idxs[i + 2]])

        frames = self.decode_selected_frames(vr, sampled_idxs_new, video_face_info_d)
