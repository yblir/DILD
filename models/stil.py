...
class STIL_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # for item in x[0]:
        #     item = item.cpu().numpy()
        #     img_numpy.append(item.transpose(2, 0, 1))
        # img_numpy = np.concatenate(img_numpy, axis=0)
        # x = torch.from_numpy(img_numpy).float().cuda(0).unsqueeze(0)

        # x: [n, tc, h, w] -> [nt, c, h, w]
        # out: [nt, num_class]
        out = self.base_model(
                x.reshape((-1, img_channel) + x.size()[2:])
        )

        out = out.reshape(-1, self.num_segment, self.num_class)  # [n, t, num_class]
